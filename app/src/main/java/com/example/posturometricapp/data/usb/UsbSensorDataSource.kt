...
/**
 * Источник данных с USB: устанавливает соединение, читает байты,
 * парсит пакеты, обрабатывает калибровку и эмитит чистые данные.
 */
class UsbSensorDataSource(private val context: Context) {

    private val connectionManager = UsbConnectionManager(context)
    private val parser = SensorPacketParser()
    private val calibrationEngine = CalibrationEngine()
    private var serialPort: UsbSerialPort? = null

    /**
     * Начинает прослушивание USB и возвращает Flow<SensorDataUsb>.
     */
    fun startListening(): Flow<SensorDataUsb> = callbackFlow {
        // Запрашиваем и открываем порт
        serialPort = connectionManager.openFirstPort()
        if (serialPort == null) {
            UsbErrorNotifier.notify(UsbError("Failed to open USB port", null))
            close()
            return@callbackFlow
        }

        // Запускаем ioManager для чтения
        val ioManager = SerialInputOutputManager(serialPort, object : SerialInputOutputManager.Listener {
            override fun onNewData(data: ByteArray?) {
                data?.let {
                    val chunk = String(it)
                    // Парсим полный пакет
                    parser.feed(chunk)?.let { packet ->
                        parser.parse(packet)?.let { rawData ->
                            val calibrated = calibrationEngine.offsets?.let { rawData.sensorValues }
                                ?.let { calibrationEngine.applyCalibration(it) }
                                ?: rawData.sensorValues

                            // Эмитим скорректированный объект
                            trySend(rawData.copy(sensorValues = calibrated)).isSuccess
                        }
                    }
                }
            }
            ...
            override fun onRunError(e: Exception?) {
                launch { UsbErrorNotifier.notify(UsbError(e?.localizedMessage ?: "IOManager error", e)) }
                close()
            }
        })

        // Запускаем чтение в IO
        val job = launch(Dispatchers.IO) {
            try {
                ioManager.run()
            } catch (e: Exception) {
                launch { UsbErrorNotifier.notify(UsbError("Read loop failed", e)) }
            }
        }

        awaitClose {
            job.cancel()
            serialPort?.let { connectionManager.closePort(it) }
        }
    }

    /**
     * Инициирует калибровку: сбрасываем накопитель и начинаем сбор.
     */
    ...
    /**
     * Отправляет простую команду на порт.
     */
    private suspend fun sendCommand(command: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            launch { UsbErrorNotifier.notify(UsbError("Command send failed: $command", e)) }
            false
        }
    }
